...
// Request type with optional route - use intersection to avoid conflicts
...
// Process info for metrics collection
...
export async function collectChildProcessMetrics(
...
export function trackProcessRestart(
  analysisId: string,
  reason = 'unknown',
): void {
...
export function trackProcessError(
  analysisId: string,
  errorType = 'runtime',
): void {
...
export function trackLogLine(analysisId: string): void {
  analysisLogLines.inc({ analysis_id: analysisId });
}

export function trackIPCMessage(
  analysisId: string,
  direction = 'inbound',
): void {
...
export function trackDNSCache(analysisId: string, hit = true): void {
  if (hit) {
