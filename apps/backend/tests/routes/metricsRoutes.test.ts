...
describe('Metrics Routes - WITH REAL AUTH', () => {
  ...
  beforeEach(async () => {
    ...
    // Import metrics register for verification (cast through unknown for mocked types)
    const metricsModule = await import('../../src/utils/metrics-enhanced.ts');
    register = metricsModule.register as unknown as MetricsRegister;

    // Import routes with REAL auth middleware
    const { metricsRouter } = await import('../../src/routes/metricsRoutes.ts');
