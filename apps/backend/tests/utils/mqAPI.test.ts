...
type MQAPIModule = {
  default: {
    ...
    getAPICall: (endpoint: string, token: string) => Promise<APIResponse>;
    getDevices: (token: string) => Promise<APIResponse>;
    getGateways: (token: string) => Promise<APIResponse>;
    getAccount: (token: string) => Promise<APIResponse>;
    createDevice: (
      token: string,
