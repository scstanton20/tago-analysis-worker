...
interface SSEModule {
  sseManager: {
    ...
    broadcastUpdate: (type: string, data: Record<string, unknown>) => void;
    sendHeartbeat: () => void;
    broadcastMetricsUpdate: () => Promise<void>;
...
describe('SSE Channel-Based Subscription Management', () => {
  ...
  let betterSSE: { createSession: Mock; createChannel: Mock };
  ...
  beforeEach(async () => {
    ...
    betterSSE = (await import('better-sse')) as unknown as {
      createSession: Mock;
      createChannel: Mock;
    };
    ...
    betterSSE.createChannel.mockImplementation(() => {
      ...
      channel.register.mockImplementation(function (
        this: MockChannel,
        session: MockSession,
      ) {
      ...
      channel.deregister.mockImplementation(function (
        this: MockChannel,
        session: MockSession,
      ) {
        // Use 'this' to ensure we're modifying the correct channel object
  ...
  describe('Broadcasting Behavior', () => {
    ...
    describe('global broadcasts (non-log)', () => {
      it('should send init to global channel (all sessions)', async () => {
        ...
        // Prevent unused variable warning - session needed to set up SSE client
        void session1;
  ...
  describe('Integration with Existing SSEManager', () => {
    ...
    describe('sendInitialData compatibility', () => {
      it('should still send initial data on connection', async () => {
        ...
        (teamService.getAllTeams as Mock).mockResolvedValue([
          { id: 'team-1', name: 'Team 1' },
        ]);
        (analysisService.getConfig as Mock).mockResolvedValue({
          teamStructure: {},
        });
