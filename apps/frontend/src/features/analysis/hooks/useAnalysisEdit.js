...
export function useAnalysisEdit({
  ...
  useEffect(() => {
    ...
    async function checkFormat() {
      // Use web worker to check format changes (off main thread)
      const { checkFormatChanges } = await import('@/workers/codeWorkerClient');
      const result = await checkFormatChanges(content);
      if (!isCancelled) {
        setHasFormatChanges(result.hasChanges);
      }
    }
  ...
  const handleSave = async () => {
    const result = await saveOperation.execute(async () => {
      ...
      if (isEnvMode) {
        ...
        // Auto-format JavaScript before saving (using web worker)
        try {
          const { formatCode } = await import('@/workers/codeWorkerClient');
          const result = await formatCode(content);
          contentToSave = result.success ? result.formatted : content;
          if (!result.success) {
            logger.warn('Formatting failed, saving unformatted:', result.error);
          }
        } catch (formatError) {
