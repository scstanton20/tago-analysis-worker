...
RUN corepack enable && pnpm install --filter backend --filter @tago-analysis-worker/types --frozen-lockfile

# Copy pre-built shared types package (built by CI before Docker)
COPY packages/types/dist ./packages/types/dist

FROM node:24-alpine@sha256:eb7e57db68d1b59e87cb22ae943d2826aa6b0d3a4175a23ca965f49526a1a664 AS run
